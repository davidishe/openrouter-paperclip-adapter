...
import type { OpenRouterAdapterConfig, OpenRouterMessage, OpenRouterSessionState } from "./types.js";

const ADAPTER_TYPE = "openrouter" as const;
...
function normalizeSession(sessionParams: Record<string, unknown> | null): OpenRouterSessionState {
  ...
  const candidate = sessionParams as Partial<OpenRouterSessionState>;
...
async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    ...
    await ctx.onLog(
      "stdout",
      `[openrouter] Completed. Model: ${result.model}. Tokens: ${result.usage.totalTokens} (prompt: ${result.usage.promptTokens}, completion: ${result.usage.completionTokens})${result.costUsd != null ? `. Cost: $${result.costUsd.toFixed(6)}` : ""}\n`,
    );
